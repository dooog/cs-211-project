...
int computer::createFile(char *n, int len, char *ff){
	...
	if (newDisk->findNBlocks((len+63)/64, maximum_number_of_files/4) == -1){
		return NOT_ENOUGH_SPACE;
	} else tempFD->firstBlockNum = newDisk->findNBlocks((len+63)/64, maximum_number_of_files/4);
	putFileDescriptor(findFreeFileDescriptor(), tempFD);
	for (int i = 0; i < (len+63)/64; i++){
		char *tempfile = (char *)malloc(64);
		memcpy(tempfile, ff + i*64, 64);
		newDisk->storeBlock(tempfile, tempFD->firstBlockNum + i);
		free( (void *)tempfile);
	}
	delete tempFD;
	return 0;
...
void computer::printFiles(int argc, char *argv[]){
	...
	for (int i = 2; i < argc; i++){
		if (findFile(argv[i], tempfd) == -1) cout << "This file was not found on that machine.\n";
		else printOneFile(*tempfd);
	}
	delete tempfd;
}
...
void computer::getFileDescriptor(int n, fileDescriptor *fd){// n is the number of the file descriptor
...
int computer::findFile(char *fname, fileDescriptor *fd){
	for (int i = 0; i < maximum_number_of_files; i++){
		getFileDescriptor(i, fd);
		if (strcmp(fd->fileName, fname) == 0) return i;
	}
	return -1;
}

void computer::printOneFile(fileDescriptor fd){
	char *temptext = (char *)malloc(64);
	for (int i = 0; i < fd.fileLength/64 + 1; i++){
		memset(temptext, '\0', 64);
		newDisk->retrieveBlock(temptext, fd.firstBlockNum + i);
		for (int j = 0; j < fd.fileLength - i*64; j++){
			if (j < 64) cout << temptext[j];
		}
	}
	delete temptext;
}
